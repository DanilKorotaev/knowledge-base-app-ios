...
**Связи:** `task-feature-api-client-version-metadata.md`, `docs/FASTLANE.md`, `.github/workflows/deploy-testflight.yml`, `fastlane/Fastfile`, `project.yml`.
...
- [ ] Выбрать вариант A или A+B (зафиксировать в `docs/FASTLANE.md`).
- [ ] Добавить `VERSION` (стартовое значение, согласовать с текущим TestFlight).
- [ ] Добавить `CHANGELOG.md` (перенести ключевые изменения последних недель ретроспективно — по желанию).
- [ ] Добавить `docs/RELEASES.md` (шаблон таблицы).

### Фаза 2 — CI / Fastlane

- [ ] `fastlane beta`: `get_version_number` ← читать из `VERSION`; не полагаться на застывший `project.yml`.
- [ ] Скрипт sync: `VERSION` → `MARKETING_VERSION` в `project.yml` + `xcodegen` (если нужно).
- [ ] После upload: обновлять `docs/RELEASES.md` + `fastlane/test_output/ci_testflight.json` (уже есть marketing_version).
- [ ] Git tag `ios/v*` на success deploy (нужен `contents: write` для GITHUB_TOKEN или PAT).

### Фаза 3 — PR / качество

- [ ] CI check: при diff в `KnowledgeBaseApp/**` требовать изменение `VERSION` или `CHANGELOG.md`.
- [ ] Обновить `.github/pull_request_template.md` — секция Version & Changelog.
- [ ] Документировать правила bump в `docs/RELEASE_PROCESS.md`.

### Фаза 4 — продукт

- [ ] Экран «О приложении»: `Version {VERSION} ({BUILD})` — копируемо для саппорта.
- [ ] Связать с `task-feature-api-client-version-metadata`.
