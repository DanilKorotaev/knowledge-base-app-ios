...
**Формат событий (черновик):** каждое событие — один или несколько строк `data:`; тело — UTF-8 текст **или** одна JSON-строка на событие, например `{"delta":"…"}` или `{"done":true,"messages":[…]}`. Клиент в репозитории уже содержит парсер SSE (`SSEventParser`) для сборки `data` из потока; согласование полей JSON — при реализации FastAPI.

**Завершение:** отдельное событие `data: {"done":true}` **или** закрытие соединения после финального JSON с полным тредом — выбрать один стиль и описать в OpenAPI.
